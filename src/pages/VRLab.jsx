...
import initialContent from '../data/vrLabContent.json';
...
export default function VRLab({ onNavigate, isDarkTheme, onBookDemo, onToggleTheme }) {
    ...
    useEffect(() => {
        ...
        fetch('/api/vrlab-live')
            ...
            .catch(err => console.error('[VRLab] Error fetching live data:', err))
    ...
    return (
        <>
            ...
            <div className={`vr-product-container ${themeClass}`}>
                <section className="vr-product-hero" style={{ backgroundImage: `linear-gradient(rgba(0, 0, 0, ${isDarkTheme ? '0.7' : '0.5'}), rgba(0, 0, 0, ${isDarkTheme ? '0.7' : '0.5'})), url('/assets/vr_lab_hero.webp')` }}>
                ...
                <section className="vr-product-section">
                    <h2 className="vr-product-section-title">{content.theLab.title}</h2>
                    <p className="vr-product-section-subtitle">{content.theLab.subtitle}</p>
                    <div className="vr-product-feature-grid">
                        {content.theLab.features.map((feature, idx) => (
