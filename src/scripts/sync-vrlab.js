...
// PLACE YOUR GOOGLE DOC ID HERE for VR Lab
const GOOGLE_DOC_ID = '1AvxmahzbQrgae1PCqs0k2XWfyTJUVMeXkoYU0w-6xYk';

export async function syncVRLabContent(shouldWrite = true) {
    const rootDir = process.cwd();
    const JSON_FILE = path.join(rootDir, 'src/data/vrLabContent.json');

    if (!GOOGLE_DOC_ID || GOOGLE_DOC_ID === 'YOUR_VRLAB_GOOGLE_DOC_ID_HERE') {
        return null;
    }
    ...
    try {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (line.includes('--- HERO ---')) currentSection = 'hero';
                ...
                if (currentSection === 'hero') {
                    ...
                    if (k === 'title') data.theLab.title = v;
                    if (k === 'subtitle') data.theLab.subtitle = v;
                    if (k.startsWith('feature')) {
                        const parts = v.split('|');
                        if (parts.length >= 2) {
                            data.theLab.features.push({
                                number: parts[0].trim(),
                                title: parts[1].trim(),
        ...
        console.error('[Auto-Sync-VRLab] Error:', err.message);
        return null;
    }
}
