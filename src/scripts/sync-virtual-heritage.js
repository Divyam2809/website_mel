...
// PLACE YOUR GOOGLE DOC ID HERE for Virtual Heritage
const GOOGLE_DOC_ID = '1U_-qoY3-s_dOwNLt-x5ZYKazmqp4wnaTMipoSS-dMoc';

export async function syncVirtualHeritageContent(shouldWrite = true) {
    const rootDir = process.cwd();
    const JSON_FILE = path.join(rootDir, 'src/data/virtualHeritageContent.json');

    if (!GOOGLE_DOC_ID || GOOGLE_DOC_ID === 'YOUR_VIRTUAL_HERITAGE_GOOGLE_DOC_ID_HERE') {
        console.log('[HeritageSync] No valid Doc ID provided.');
        return null;
    }

    console.log('[HeritageSync] Starting sync for:', GOOGLE_DOC_ID);
    ...
    try {
        ...
        if (shouldWrite && Object.keys(data.hero).length > 0) {
            ...
            console.log('[HeritageSync] Data saved to:', JSON_FILE);
        }
        return Object.keys(data.hero).length > 0 ? data : null;
    } catch (err) {
        console.error('[Auto-Sync-VirtualHeritage] Error:', err.message);
        return null;
    }
}
