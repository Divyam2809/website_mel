...
// PLACE YOUR GOOGLE DOC ID HERE for VR Live Stream
const GOOGLE_DOC_ID = '1vNDtCzj9PvZjX5v0zNOMQH-nN8OlLqntIZx0uQIW8BU';

export async function syncVRLiveStreamContent(shouldWrite = true) {
    const rootDir = process.cwd();
    const JSON_FILE = path.join(rootDir, 'src/data/vrLiveStreamContent.json');

    if (!GOOGLE_DOC_ID || GOOGLE_DOC_ID === 'YOUR_VR_LIVESTREAM_GOOGLE_DOC_ID_HERE') {
        console.log('[LiveStreamSync] No valid Doc ID provided.');
        return null;
    }

    console.log('[LiveStreamSync] Starting sync for:', GOOGLE_DOC_ID);
    ...
    try {
        ...
        if (shouldWrite && Object.keys(data.hero).length > 0) {
            ...
            console.log('[LiveStreamSync] Data saved to:', JSON_FILE);
        }
        return Object.keys(data.hero).length > 0 ? data : null;
    } catch (err) {
        console.error('[Auto-Sync-VRLiveStream] Error:', err.message);
        return null;
    }
}
