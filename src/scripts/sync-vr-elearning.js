...
// PLACE YOUR GOOGLE DOC ID HERE for VR E-Learning
const GOOGLE_DOC_ID = '1-t7kNTIZGLqCo46_cO_lEPYkxtnFkX5vruydcdTUwgk';

export async function syncVRELearningContent(shouldWrite = true) {
    const rootDir = process.cwd();
    const JSON_FILE = path.join(rootDir, 'src/data/vrElearningContent.json');

    if (!GOOGLE_DOC_ID || GOOGLE_DOC_ID === 'YOUR_VRELEARNING_GOOGLE_DOC_ID_HERE') {
        return null;
    }
    ...
    try {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (line.includes('--- HERO ---')) currentSection = 'hero';
                ...
                if (currentSection === 'hero') {
                    ...
                    if (k === 'title') data.learningExperience.title = v;
                    if (k === 'subtitle') data.learningExperience.subtitle = v;
                    if (k.startsWith('feature')) {
                        const parts = v.split('|');
                        if (parts.length >= 2) {
                            data.learningExperience.features.push({
                                number: parts[0].trim(),
                                title: parts[1].trim(),
        ...
        console.error('[Auto-Sync-VRElearning] Error:', err.message);
        return null;
    }
}
