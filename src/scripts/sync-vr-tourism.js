...
// PLACE YOUR GOOGLE DOC ID HERE for VR Tourism
const GOOGLE_DOC_ID = '18biLhcxvncanh3157VQU-p_cnFANF04AjTWHthGT79w';

export async function syncVRTourismContent(shouldWrite = true) {
    const rootDir = process.cwd();
    const JSON_FILE = path.join(rootDir, 'src/data/vrTourismContent.json');

    if (!GOOGLE_DOC_ID || GOOGLE_DOC_ID === 'YOUR_VR_TOURISM_GOOGLE_DOC_ID_HERE') {
        console.log('[TourismSync] No valid Doc ID provided.');
        return null;
    }

    console.log('[TourismSync] Starting sync for:', GOOGLE_DOC_ID);
    ...
    try {
        ...
        if (shouldWrite && Object.keys(data.hero).length > 0) {
            ...
            console.log('[TourismSync] Data saved to:', JSON_FILE);
            ...
            try {
                ...
                if (apiRes.ok) console.log('[TourismSync] Live content updated in database via API.');
                else console.error('[TourismSync] API Error:', await apiRes.text());
            } catch (apiErr) {
                console.error('[TourismSync] Failed to push to API:', apiErr.message);
                console.log('Ensure backend server is running on port 3000');
            }
        }
        return Object.keys(data.hero).length > 0 ? data : null;
    } catch (err) {
        console.error('[Auto-Sync-VRTourism] Error:', err.message);
        return null;
    }
}
