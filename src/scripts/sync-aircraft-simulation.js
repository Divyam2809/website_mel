...
// PLACE YOUR GOOGLE DOC ID HERE for Aircraft Simulation
const GOOGLE_DOC_ID = '1XRqJeZ9-eOCx0RQiueRCS3z9pOz4Taxkg_auoo6t3bA';

export async function syncAircraftSimulationContent(shouldWrite = true) {
    const rootDir = process.cwd();
    const JSON_FILE = path.join(rootDir, 'src/data/aircraftSimulationContent.json');

    if (!GOOGLE_DOC_ID || GOOGLE_DOC_ID === 'YOUR_AIRCRAFT_SIMULATION_GOOGLE_DOC_ID_HERE') {
        return null;
    }
    ...
    try {
        ...
        console.error('[Auto-Sync-AircraftSimulation] Error:', err.message);
        return null;
    }
}
