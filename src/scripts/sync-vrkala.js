...
// PLACE YOUR GOOGLE DOC ID HERE for VR Kala
const GOOGLE_DOC_ID = '1GNDWwI07ZtBkp3LE0VnBK8WrQ2urXvxOPMs7rsyM7xA';

export async function syncVRKalaContent(shouldWrite = true) {
    const rootDir = process.cwd();
    const JSON_FILE = path.join(rootDir, 'src/data/vrKalaContent.json');

    if (!GOOGLE_DOC_ID || GOOGLE_DOC_ID === 'YOUR_VRKALA_GOOGLE_DOC_ID_HERE') {
        return null;
    }
    ...
    try {
        ...
        const data = {
            hero: {},
            immersionTrio: { features: [] },
            impactPillars: { pillars: [] },
            benefits: { features: [] },
            stats: { items: [] }
        };
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (line.includes('--- HERO ---')) currentSection = 'hero';
                ...
                if (currentSection === 'hero') {
                    ...
                    if (k === 'title') data.immersionTrio.title = v;
                    if (k === 'subtitle') data.immersionTrio.subtitle = v;
                    if (k.startsWith('feature')) {
                        const parts = v.split('|');
                        if (parts.length >= 2) {
                            data.immersionTrio.features.push({
                                number: parts[0].trim(),
                                title: parts[1].trim(),
                    ...
                    if (k === 'title') data.impactPillars.title = v;
                    if (k === 'subtitle') data.impactPillars.subtitle = v;
                    if (k.startsWith('pillar')) {
                        const parts = v.split('|');
                        if (parts.length >= 2) {
                            data.impactPillars.pillars.push({
                                num: parts[0].trim(),
                                title: parts[1].trim(),
                                desc: parts.slice(2).join('|').trim()
                            });
        ...
        console.error('[Auto-Sync-VRKala] Error:', err.message);
        return null;
    }
}
