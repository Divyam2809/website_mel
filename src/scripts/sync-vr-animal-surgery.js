...
// PLACE YOUR GOOGLE DOC ID HERE for VR Animal Surgery
const GOOGLE_DOC_ID = '1QmbuU_yrc46-Se-ktK4WsiBCbXxX2IIlNKlAPCI45Oo';

export async function syncVRAnimalSurgeryContent(shouldWrite = true) {
    const rootDir = process.cwd();
    const JSON_FILE = path.join(rootDir, 'src/data/vrAnimalSurgeryContent.json');

    if (!GOOGLE_DOC_ID || GOOGLE_DOC_ID === 'YOUR_VR_ANIMAL_SURGERY_GOOGLE_DOC_ID_HERE') {
        return null;
    }
    ...
    try {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (line.includes('--- HERO ---')) currentSection = 'hero';
                ...
                if (currentSection === 'hero') {
                    ...
                    if (k === 'title') data.surgicalSuite.title = v;
                    if (k === 'subtitle') data.surgicalSuite.subtitle = v;
                    if (k.startsWith('feature')) {
                        const parts = v.split('|');
                        if (parts.length >= 2) {
                            data.surgicalSuite.features.push({
                                number: parts[0].trim(),
                                title: parts[1].trim(),
        ...
        console.error('[Auto-Sync-VRAnimalSurgery] Error:', err.message);
        return null;
    }
}
